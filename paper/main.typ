...
Byte-level models @xue2022byt5 @kalchbrenner2016bytenet eliminate the tokenizer but pay a quadratic attention cost on sequences that are three to five times longer than subword sequences. Structured state space models @gu2022s4 @gu2023mamba achieve linear-time sequence processing through learned state transitions, but operate on token embeddings rather than raw signal.
...
The sketch provides a 64-bit state fingerprint updated by one XOR per input bit. It inherits its mechanism from Zobrist hashing @zobrist1970hashing, originally developed for incremental board-position hashing in game-playing programs.
...
*Tokenization.* Subword tokenization @sennrich2016bpe @kudo2018sentencepiece remains the dominant paradigm for preparing text input to neural models. Both BPE and SentencePiece learn a discrete vocabulary from training data, creating a language-specific preprocessing stage. Byte-level models @xue2022byt5 @kalchbrenner2016bytenet eliminate the vocabulary but incur longer sequences and higher attention costs. The FSVM differs from both: it extracts structural features from a boolean stream using a fixed state machine, with no learned vocabulary and no neural preprocessing.

*Streaming algorithms.* The FSVM's O(1) per-step processing connects to the streaming algorithms literature established by Alon et al.'s AMS sketch @alon1996ams and the Count-Min Sketch @cormode2005countmin, surveyed by Muthukrishnan @muthukrishnan2005streams. These randomized data structures approximate aggregate statistics over streams with bounded memory. The FSVM shares the commitment to bounded work per symbol but is deterministic and tracks structural properties rather than frequency statistics.

*State space models.* Structured state space models @gu2022s4 @gu2023mamba achieve linear-time sequence processing through continuous-time state transitions with learned parameters. The FSVM shares the state-based paradigm but operates on raw bits with discrete, bounded state (56 bytes) and geometry-defined (not learned) transitions.
